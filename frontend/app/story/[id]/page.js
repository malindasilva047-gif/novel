...
function estimateReadTime(chapters) {
  const totalWords = (chapters || []).reduce((sum, ch) => sum + Number(ch?.word_count || 0), 0);
  const minutes = Math.max(1, Math.round(totalWords / 200));
  const h = Math.floor(minutes / 60);
  const m = minutes % 60;
  if (!h) return `${m}m read time`;
  return `${h}h ${m}m read time`;
}
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const [storyRes, chaptersRes, commentsRes, recoRes, historyRes] = await Promise.all([
  ...
  return (
    <main className="story-page-root">
      ...
      <section className="hero-section">
        <div className="cover-col">
          ...
          <div className="cover-actions">
            ...
            <button className="btn-secondary" type="button">＋ Add to Library</button>

            <div className="mini-actions">
              <button className="mini-btn" type="button">♥ Like</button>
              <button className="mini-btn" type="button">⚡ Save</button>
              <button className="mini-btn" type="button">↗ Share</button>
            </div>
          </div>
        </div>
        ...
        <div className="story-col">
          ...
          <div className="author-row">
            <div className="author-avatar">{String(author?.name || story?.author_name || 'A').slice(0, 1).toUpperCase()}</div>
            <div className="author-name-wrap">
              <div className="author-name">{author?.name || story?.author_name || 'Unknown Author'}</div>
          ...
          <div className="story-desc">
            <p className={descExpanded ? '' : 'desc-truncated'}>{story?.description || 'No description available.'}</p>
        ...
        <aside className="right-col">
          <div className="author-card-side">
            ...
            <div className="author-card-body">
              <div className="author-card-avatar">{String(author?.name || story?.author_name || 'A').slice(0, 1).toUpperCase()}</div>
              <div className="author-card-name">{author?.name || story?.author_name || 'Unknown Author'}</div>
              <div className="author-card-handle">@{String(author?.name || story?.author_name || 'author').toLowerCase().replace(/\s+/g, '_')}</div>
              <p className="author-card-bio">{author?.bio || 'Story writer and publisher profile on NovelReads.'}</p>
      ...
      <style jsx>{`
        ...
        .story-page-root { position: relative; min-height: calc(100vh - 60px); padding-bottom: 40px; }
        .hero-bg {
        ...
        .cover-col { position: sticky; top: 82px; display: flex; flex-direction: column; gap: 12px; }
        ...
        .author-row { display: flex; align-items: center; gap: 10px; margin-bottom: 18px; }
        .author-avatar {
          width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg,#7c6df0,#f43f5e);
          color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700;
        }
        .author-name { color: #e8eaf0; font-weight: 700; font-size: 14px; }
        ...
        .story-desc { color: #9aa3b8; line-height: 1.8; border-left: 2px solid rgba(124,109,240,0.3); padding-left: 16px; }
        ...
        .author-card-name { color: #fff; font-weight: 700; margin-top: 8px; }
        .author-card-handle { color: #5a6480; font-size: 12px; margin-bottom: 8px; }
        ...
        @media (max-width: 780px) {
          .hero-section, .tabs-section { grid-template-columns: 1fr; }
          .cover-col { position: static; width: 100%; max-width: 260px; }
